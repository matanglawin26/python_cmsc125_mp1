...
class Simulation:
    ...
    def initialize(self, users):
        for user in users:
            # print("USER CURR_REQ: ",user.curr_req())
            # print("IS USER %s COMPLETE? %s" % (user, user.is_complete()))
        ...
        print("AFTER INIT: ",self.__process)
    ...
    def in_process(self, curr_req, curr_user):
        return any(1 if curr_req.id == req.id and curr_user.id != user_id else 0 for (user_id,req) in self.__process.items())
    
    def remove(self, curr_user_id):
        self.__process.pop(curr_user_id)
