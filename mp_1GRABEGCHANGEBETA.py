...
class Queue:
    ...
    def dequeue(self,req_id, curr_user):
        ...
        for i in range(len(curr_req)):
            (item_key, item_value), = curr_req[i].items()
            if item_key.id == curr_user.id:
                self.__queue[req_id].pop(i)
                break
...
class Simulation:
    ...
    def add_process(self, curr_user, req):
        self.__process[curr_user] = req
